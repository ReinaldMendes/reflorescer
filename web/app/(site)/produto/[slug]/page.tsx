...
export default async function ProductPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      price: product.price,
      availability:
        (product.inventory?.quantity ?? 0) > 0 ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
    },
  };
        ...
        <div>
          ...
          <div className="mt-4 flex items-baseline gap-3">
            {product.compareAtPrice && Number(product.compareAtPrice) > Number(product.price) && (
              <span className="text-lg text-brand-300 line-through">{formatCurrency(product.compareAtPrice)}</span>
            )}
            <span className="text-2xl font-medium text-brand-800">{formatCurrency(product.price)}</span>
          </div>
          ...
          <AddToCartForm productId={product.id} variants={product.variants} inStock={(product.inventory?.quantity ?? 0) > 0} />
