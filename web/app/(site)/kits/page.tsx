import { listKits } from "@/lib/api-client";
import Image from "next/image";
import Link from "next/link";
import { formatCurrency } from "@/lib/utils";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Kits", description: "Kits presenteáveis, de autocuidado e aromáticos da Reflorescer." };
      ...
      <div className="grid grid-cols-2 gap-6 lg:grid-cols-3">
        {kits.map((kit: any) => (
          <Link key={kit.id} href={`/kits/${kit.slug}`} className="group block">
            ...
            <p className="text-brand-600">{formatCurrency(kit.price)}</p>
          </Link>
        ))}
