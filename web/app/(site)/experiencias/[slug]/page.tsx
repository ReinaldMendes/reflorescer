import { getExperienceBySlug } from "@/lib/api-client";
import { notFound } from "next/navigation";
import { ProductCard } from "@/components/site/product-card";

export default async function ExperiencePage({ params }: { params: { slug: string } }) {
  const experience = await getExperienceBySlug(params.slug).catch(() => null);
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-16 lg:px-12">
      ...
      <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">
        {experience.products.map(({ product }: any) => (
          <ProductCard key={product.id} product={{ id: product.id, name: product.name, slug: product.slug, price: product.price, images: product.images }} />
        ))}
