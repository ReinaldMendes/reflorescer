import { listBlogPosts } from "@/lib/api-client";
import Link from "next/link";
import Image from "next/image";
import { formatDate } from "@/lib/utils";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Diário Reflorescer", description: "Reflexões, dicas de autocuidado e bastidores do fazer artesanal." };
        ...
        {posts.map((post: any) => (
          <Link key={post.id} href={`/journal/${post.slug}`} className="group grid gap-6 sm:grid-cols-2">
            <div className="relative aspect-[4/3] overflow-hidden rounded-organic bg-brand-100">
              {post.coverImage && <Image src={post.coverImage} alt={post.title} fill className="object-cover transition-transform duration-organic group-hover:scale-105" />}
            </div>
            <div>
